...
import {
  ...
  CheckCircle, AlertCircle, Sparkles, ChevronDown, ChevronUp, X,
} from 'lucide-react';
...
export default function ConversationPage() {
  ...
  const [correctionOpen, setCorrectionOpen] = useState(false); // mobile toggle
  ...
  useEffect(() => {
    ...
    startConversation(settings.voiceType ? undefined : undefined, topic)
      .then((reply) => {
  ...
  const handleSend = useCallback(async (text: string) => {
    ...
    try {
      const result = await sendMessage(undefined, topic, [...messages, userMsg], text.trim());
  ...
  }, [isLoading, topic, settings.autoSpeak, messages, isMuted, speak, stopListening, resetTranscript]);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950">
      ...
      {/* ── Main body: chat left + correction right (desktop) ── */}
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            {messages.map((msg) => (
              <MessageBubble
                key={msg.id}
                message={msg}
                onShowAnalysis={(a) => { setActiveAnalysis(a); setCorrectionOpen(true); }}
              />
            ))}
          ...
          {/* Mobile: correction toggle bar */}
          {activeAnalysis && (
            <button
              className="lg:hidden flex items-center justify-between px-4 py-2.5 border-t border-gray-800 bg-gray-900 text-sm"
              onClick={() => setCorrectionOpen((o) => !o)}
            >
              <div className="flex items-center gap-2">
                {activeAnalysis.hasErrors
                  ? <AlertCircle size={15} className="text-yellow-400" />
                  : <CheckCircle size={15} className="text-green-400" />}
                <span className="text-gray-300">교정 결과</span>
                <ScorePill score={activeAnalysis.score} />
              </div>
              {correctionOpen ? <ChevronDown size={15} className="text-gray-500" /> : <ChevronUp size={15} className="text-gray-500" />}
            </button>
          )}

          {/* Mobile: correction panel */}
          {activeAnalysis && correctionOpen && (
            <div className="lg:hidden border-t border-gray-800 max-h-64 overflow-y-auto">
              <CorrectionContent analysis={activeAnalysis} onClose={() => setCorrectionOpen(false)} />
            </div>
          )}
        ...
        {/* ── RIGHT: Correction panel (desktop only) ── */}
        <div className="hidden lg:flex flex-col w-80 xl:w-96 border-l border-gray-800 bg-gray-900/50 shrink-0 overflow-y-auto">
          {activeAnalysis ? (
            <CorrectionContent analysis={activeAnalysis} />
          ) : (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-6">
              <div className="w-14 h-14 rounded-2xl bg-violet-900/30 flex items-center justify-center text-2xl">✏️</div>
              <p className="text-sm text-gray-400 font-medium">교정 패널</p>
              <p className="text-xs text-gray-600 leading-relaxed">
                메시지를 보내면 여기에<br />문법 교정과 피드백이 표시됩니다
              </p>
            </div>
          )}
        </div>
...
function MessageBubble({ message, onShowAnalysis }: { message: Message; onShowAnalysis: (a: Analysis) => void }) {
...
function CorrectionContent({ analysis, onClose }: { analysis: Analysis; onClose?: () => void }) {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {/* Feedback */}
      <p className="text-sm text-gray-400 italic leading-relaxed">{analysis.feedback}</p>
      ...
      {analysis.errors.map((err, i) => (
        <div key={i} className="space-y-2">
          <div className="flex items-center gap-2">
            <span className={clsx('text-xs px-2 py-0.5 rounded-full border font-medium', errorTypeBg(err.type))}>
